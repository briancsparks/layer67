...
/**
 *  Configures an instance to perform as a web-tier instance.
 *
...
lib.buildServerConfig = function() {
  ...
  var ra = raLib.adapt(arguments, (argv, context, callback) => {
    ...
    const routerPort          = argvGet(argv, 'port')                       || 5776;
    const webtierRouter       = argvGet(argv, 'webrouter')                  || `http://127.0.0.1:${port}`;
    ...
    setOnn(configJson_, 'revProxyTimeout',       revProxyTimeout);
    setOnn(configJson_, 'logfile',               logfile);
    setOnn(configJson_, 'routerPort',            routerPort);
