 ...
 */
lib.cmds.configweb = function(req, res, url, restOfPathParts) {
...
lib.buildServerConfig = function() {
  ...
  var ra = raLib.adapt(arguments, (argv, context, callback) => {
    ...
    const fqdn                = argvGet(argv, 'fqdn')             || 'layer67.mobilewebassist.net';
    ...
    const isDefault           = argvGet(argv, 'default')              || "";
    const clientCert          = argvGet(argv, 'client-cert,client')   || path.join('/etc/nginx/certs', fileFqdn+'-client-root.cer');
    const certfile            = argvGet(argv, 'certfile')             || path.join('/etc/nginx/certs', fileFqdn+'.cer');
    const keyfile             = argvGet(argv, 'keyfile')              || path.join('/etc/nginx/certs', fileFqdn+'.key');
