      ...
      return sg.__eachll(templates, function(item, next) {
        ...
        return fs.writeFile(tmpFilename, JSON.stringify(template, null, 2), function(err) {
          ...
          return cf.createStack(params, function(err, data) {
            ...
            var inProgress, created, completeAt, complete;
            return sg.until(function(again, last, count, elapsed) {
              ...
              return sg.__runll([function(next) {
                return cf.describeStackEvents({StackName}, function(err, data) {
                  ...
                  inProgress = sg.numKeys(      _.pluck(_.filter(data.StackEvents, event => event.ResourceStatus === 'CREATE_IN_PROGRESS'), 'LogicalResourceId'));
                  created    = sg.numKeys(      _.pluck(_.filter(data.StackEvents, event => event.ResourceStatus === 'CREATE_COMPLETE'), 'LogicalResourceId'));
                ...
                console.error(`Complete ${acctName} (${inProgress}/${inProgress+created})? ${complete}`);
