...
lib.createStacks = function() {
  ...
  var ra = raLib.adapt(arguments, (argv, context, callback) => {
    ...
    const sshCidrs    = (argvGet(argv, 'ssh-cidrs,ssh') || '').split(',');
    ...
    var   devSubnetCidr   = `10.${classBt}.0.0/0`;
    var   prodSubnetCidr  = `10.${classB}.0.0/0`;

    return sg.__run2({dev, prod, debug}, callback, [function(result, next, last, abort) {

      const vpcRef        = ref("VPC");
      const igwRef        = ref("InernetGateway");
      ...
      const prtRef    = itemOnnBoth("PublicRoute",      "Route",                {RouteTableId: prttblRef, GatewayId: gwRef});
                         setOnnBoth("PublicRoute.DestinationCidrBlock",         '0.0.0.0/0');
...
//        var   result;
        ...
        const subnetRef   = itemOnnBoth(`Subnet${LETTER}${type}`,            "Subnet",          {VpcId:     vpcRef});
                             ...
                             setOnnBoth([`Subnet${LETTER}Public`, 'Properties', 'AvailabilityZone'],      az(letter));

        if (type === 'Public') {
          const assocName = `Subnet${LETTER}PublicRouteTableAssociation`;

//          result = subnetRef  = itemOnnBoth(`Subnet${LETTER}Public`, "Subnet",          {VpcId:     vpcRef});
//          const subAssocRef   = itemOnnBoth(assocName, "SubnetRouteTableAssociation", {SubnetId:  subnetRef, RouteTableId: prttblRef});

//                               setOnnBoth([`Subnet${LETTER}Public`, 'Properties', 'AvailabilityZone'],    az(letter));
//                               setOnnBoth([`Subnet${LETTER}Public`, 'Properties', 'MapPublicIpOnLaunch'], true);

                                   // TODO: fix the class-c
                                   setOnn(dev,  [`Subnet${LETTER}Public`, 'Properties', 'CidrBlock'],           `10.${classBt}.0.0/19`);
                                   setOnn(prod, [`Subnet${LETTER}Public`, 'Properties', 'CidrBlock'],           `10.${classB}.0.0/19`);
        }
        ...
        if (type !== 'Public') {
          rttblRef          = itemOnnBoth(`Subnet${LETTER}${type}RouteTable`, "RouteTable",           {VpcId: vpcRef});
                              itemOnnBoth(`Subnet${LETTER}${type}PublicRoute`, "Route",           {RouteTableId: rttblRef, NatGatewayId: gatewayRef, DestinationCidrBlock: '0.0.0.0/0'});
//          const subtypeRef  = itemOnnBoth(`Subnet${LETTER}${type}`,            "Subnet",          {VpcId:     vpcRef});
//                               setOnnBoth(`Subnet${LETTER}${type}.Properties.AvailabilityZone`,   az(letter));
//                               setOnnBoth(`Subnet${LETTER}${type}.Properties.MapPublicIpOnLaunch`,false);
                               setOnnBoth(`Subnet${LETTER}${type}.Properties.CidrBlock`,          '0.0.0.0/0');

//                              itemOnnBoth(`Subnet${LETTER}${type}RouteTableAssociation`, "SubnetRouteTableAssociation", {SubnetId: subnetRef, RouteTableId: myrttblRef});
      ...
      // ---------- web sg ----------
      itemOnnBoth('sgWeb', 'SecurityGroup', {VpcId: vpcRef, GroupDescription: 'For wide use inside vpc'});
      ...
      // ---------- S3 endpoint ----------
      itemOnnBoth('S3Endpoint', 'VPCEndpoint', {VpcId: vpcRef, ServiceName: 'com.amazonaws.us-east-1.s3'});
      _.each(routeTables, table => {
        sg.setOnna(dev,  `S3Endpoint.Properties.RouteTableIds`, table);
        sg.setOnna(prod, `S3Endpoint.Properties.RouteTableIds`, table);
      });

      return next();

    }, function(result, next, last, abort) {
      result.devKeys = sg.numKeys(result.dev);
      result.prodKeys = sg.numKeys(result.prod);

      result.devlen = JSON.stringify(result.dev).length;
      result.prodlen = JSON.stringify(result.prod).length;

      //result.devkeys = _.keys(result.dev);
      //result.prodkeys = _.keys(result.prod);
...
function getAtt(ref, key) {
  return {'Fn::GetAtt':[ref[sg.firstKey(ref)].Ref, key]};
}
