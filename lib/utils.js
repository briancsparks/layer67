...
lib.tellService = function(name, location, ttl, callback) {
  ...
  const providerName  = `provider:${location}`;

  return redis.psetex(providerName, ttl, location, function(err, res) {
    return redis.sadd(serviceName, providerName, function(err, res) {
