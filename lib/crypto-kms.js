...
lib.pushCertToServer = function() {
  ...
  var ra = raLib.adapt(arguments, (argv_, context, callback) => {
    ...
    const basename        = fullpathParts.pop();
    const dirpath         = fullpathParts.join('/');
...
//
//    const storedObj       = `${project}/secrets/deploy/${baseName}.enc`;               /* The filename path of the encrypted file */
